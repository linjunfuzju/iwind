...
> ⚠️ **Recommended**: **Ubuntu 22.04** for best compatibility. Windows Docker Desktop (WSL2) has a known issue where image layer extraction may silently fail during `docker load`, causing earthquake simulation to fail without clear error messages. If you encounter earthquake cases failing, see the FAQ below.
...
**Download link:**
```
https://download.scidb.cn/download?fileId=72c2c630838955520c211c6197448744&path=/V2/Iwind simulation dataset.tar&username=linjunfu@zju.edu.cn&fileName=Iwind%20simulation%20dataset.tar
```
...
**Q: Earthquake simulation keeps failing with "FileNotFoundError"?**
> This is likely due to the `/app/earthquake/` directory missing from the Docker image. This is a known issue with Windows Docker Desktop WSL2:
> - **Cause**: WSL2 may silently fail to extract some image layers during `docker load`, even when the `.tar.gz` file SHA256 hash is correct.
> - **Symptom**: `docker run --rm zwind-reproduce:v4 ls /app/` shows no `earthquake` directory.
>
> **Solution A - Preferred (Ubuntu)**:
> 1. Use Ubuntu 22.04 to load and run the image, which does not have this issue.
>
> **Solution B - Windows workaround**:
> 1. Delete old images: `docker rmi -f $(docker images -q zwind-reproduce)`
> 2. Reload the image: `docker load -i Iwind_reproduce_v5.tar.gz`
> 3. Tag if needed: `docker tag zwind-reproduce:latest zwind-reproduce:v4`
> 4. Verify fix: `docker run --rm zwind-reproduce:v4 ls /app/earthquake/`
>
> **Solution C - Manual fix (if above doesn't work)**:
> A backup of the `earthquake/` directory is included in this project folder. If the Docker image extraction is incomplete:
> 1. Copy the `earthquake/` folder from this directory into your running container:
>    ```
>    docker cp earthquake/ <container-id>:/app/earthquake/
>    ```
>    You can find your container ID with: `docker ps -a | findstr zwind`
...
- `earthquake/` (**backup earthquake data folder** — include this folder as a workaround for Windows Docker Desktop WSL2 image extraction issues)
...
Contact the project maintainer for further questions: **linjunfu@zju.edu.cn**
