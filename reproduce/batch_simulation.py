...
def _sync_earthquake_to_container():
    """Sync local earthquake folder to container's simulation_runs/earthquake/.

    Always copies from local earthquake/ (next to this script) to
    simulation_runs/earthquake/ on the host. Since /app/simulation_runs/ is
    a volume mount, the copy becomes visible to the container immediately.

    This overwrites any existing earthquake content to ensure consistency.
    """
    import shutil
    import os

    script_dir = os.path.dirname(os.path.abspath(__file__)) if '__file__' in dir() else os.getcwd()
    local_eq = os.path.join(script_dir, "earthquake")
    host_sim_runs = os.path.join(script_dir, "simulation_runs")
    host_eq = os.path.join(host_sim_runs, "earthquake")

    if not os.path.isdir(local_eq):
        print(f"[WARNING] Local earthquake folder not found at {local_eq}")
        print(f"[HINT] Please ensure earthquake/ folder is in the same directory as batch_simulation.py")
        return False

    os.makedirs(host_sim_runs, exist_ok=True)
    print(f"[INFO] Syncing earthquake from {local_eq} to {host_eq}...")
    try:
        if os.path.exists(host_eq):
            shutil.rmtree(host_eq)
        shutil.copytree(local_eq, host_eq)
        print(f"[OK] Earthquake synced to {host_eq}")
        return True
    except Exception as e:
        print(f"[ERROR] Failed to sync earthquake: {e}")
        return False
...
def run_simulation(results_path):
    ...
    try:
        result = subprocess.run(create_cmd, capture_output=True, encoding='utf-8', errors='replace', text=True, env=env)
        if result.returncode != 0:
    ...
    # Sync earthquake data to simulation_runs/ on host
    # Always sync to ensure the local master version overwrites any corrupted container version
    print("[INFO] Syncing earthquake data to simulation_runs/...")
    if not _sync_earthquake_to_container():
        print("[ERROR] Earthquake data is not available. Batch simulation cannot proceed.")
        print("[HINT] Please ensure earthquake/ folder is in the same directory as batch_simulation.py")
